...
def will_print(level=1):
    ...
    if level == 1:
        ...
        return ((isinstance(verbosity, int) and level <= verbosity) or
                (isinstance(verbosity, bool) and verbosity))
...
def _gaussian_reduce_two_vectors(U, V, eps):
    ...
    while not done:
        if it > 10:  # pragma: no cover
            ...
            exit()
        R = [V[i] - int(round(dot(U, V) / dot(U, U) + 1E-10)) * U[i]
             for i in range(3)]  # Shorten V as much as possible
    ...
    # Make sure that the {U,V} are listed in ascending order on exit;
    # ||U||<||V||
    temp = U
    U = V
    V = temp
...
def _minkowski_conditions_check(basis, eps):
    ...
    if linalg.norm(b2) > (linalg.norm(
            [b1[i] + b2[i] for i in range(len(b1))]) + eps):
        minkowski_check = False
    ...
    if linalg.norm(b3) > (linalg.norm(
            [b1[i] + b3[i] for i in range(len(b1))]) + eps):
        minkowski_check = False
        err("Minkowski_condition 5 failed: b3 > b1+b3")

    if linalg.norm(b3) > (linalg.norm(
            [b3[i] - b1[i] for i in range(len(b1))]) + eps):
        minkowski_check = False
        err("Minkowski_condition 6 failed: b3 > b3-b1")

    if linalg.norm(b3) > (linalg.norm(
            [b2[i] + b3[i] for i in range(len(b2))]) + eps):
        minkowski_check = False
        err("Minkowski_condition 7 failed: b3 > b2+b3")

    if linalg.norm(b3) > (linalg.norm(
            [b3[i] - b2[i] for i in range(len(b2))]) + eps):
        minkowski_check = False
        err("Minkowski_condition 8 failed: b3 > b3-b2")

    if linalg.norm(b3) > (linalg.norm(
            [b1[i] + b2[i] + b3[i] for i in range(len(b2))]) + eps):
        minkowski_check = False
...
def _reduce_C_in_ABC(A, B, C, eps):
    ...
    if not allclose(dot(T, cross(A, B)), 0, atol=eps,
                    rtol=eps):  # pragma: no cover
        err("{} Projection of C into A,B plane failed".format(
            str(dot(T, cross(A, B)))))
    ...
    ABC = [A, B, C]
    ABCinv = linalg.inv(ABC)
    temp = matmul(list(map(list, zip(*ABCinv))),
                  list(map(list, zip(*oldABC)))).tolist()
    for i in range(3):
        for j in range(3):
            if abs(temp[i][j] - int(round(temp[i][j]))
                   ) > eps:  # pragma: no cover
...
def _get_lattice_parameter(elements, concentrations, default_title):
    ...
    if elements is None:
        ...
        if len(elements) != len(concentrations):
            raise ValueError(
                "You have provided {} element names when {} elements are present "
                "in the system. Please provide the correct number of elements." .format(
                    len(elements), len(concentrations)))
...
def _cartesian2direct(sLV, aBas, eps):
    ...
    for iAt in range(nAt):
        ...
        while any(aBas[iAt] >= (1.0 - eps)) or any(aBas[iAt] < (0.0 - eps)):
            aBas[iAt] = array(
                [i if i < (1.0 - eps) else i - 1.0 for i in aBas[iAt]])
            aBas[iAt] = array(
                [i if i >= (0.0 - eps) else i + 1.0 for i in aBas[iAt]])
...
def _map_enumStr_to_real_space(system_data, structure_data, minkowskiReduce):
    ...
    if minkowskiReduce:
        sLV = list(
            map(list, zip(*_minkowski_reduce_basis(list(map(list, zip(*sLV))), eps))))
    ...
    for iD in range(0, nD):
        ...
        for z1 in range(a):
            for z2 in range(int((b * z1) / a), int(c + (b * z1) / a)):
                for z3 in range(int(z1 * (d - (e * b) / c) / a + (e * z2) / c),
                                int(f + z1 * (d - (e * b) / c) / a + (e * z2) / c)):
                    ic += 1
                    ...
                    temp2 = [temp[i] + pBas[iD][i]
                             for i in range(len(pBas[iD]))]
                    aBas.append(temp2)
                    # Map position into the group
                    greal = matmul(
                        L, [float(z1), float(z2), float(z3)]).tolist()
                    # Convert the g-vector from real to integer
                    g = [int(i) for i in greal]
                    if not allclose(
                            greal, g, rtol=eps, atol=eps):  # pragma: no cover
    ...
    space_data = {"sLV": list(map(list, zip(*sLV))),
                  "aBas": aBas, "spin": spin, "gIndx": gIndx, "x": x}
...
def _minkowski_reduce_basis(IN, eps):
    ...
    if allclose(linalg.det(IN), 0.0, rtol=eps, atol=eps):
        raise ValueError(
            "Input basis for 'minkowski_reduce_basis' was not linearly independent")

    OUT = deepcopy(IN)

    # Keep applying the greedy algorithm until the vectors come out already
    # sorted
    for it in range(1, limit + 1):
        ...
        (OUT[0], OUT[1], OUT[2]) = _reduce_C_in_ABC(
            OUT[0], OUT[1], OUT[2], eps)
        ...
        if not temp.startswith("#") and "#" not in temp.split()[0]:
            ...
            if line_count - (14 + adjust) in structures:
                ...
                if len(data) == 28:
                    ...
                    this_struct["directions"] = '0' * \
                        len(this_struct["labeling"])
...
def _write_POSCAR(system_data, space_data, structure_data, args):
    ...
    def_title = "{} str #: {}\n".format(
        str(system_data["title"]), str(structure_data["strN"]))

    # Get the lattice parameter for the atomic species provided by the
    # user.
    lattice_parameter, title = _get_lattice_parameter(
        args["species"], concs, def_title)
    ...
    with open(filename, "w+") as poscar:
        ...
        for ilab in range(system_data["k"]):
            for iAt in range(structure_data["n"] * system_data["nD"]):
                ...
                displace = directions[iAt] * \
                    args["displace"] * lattice_parameter
                ...
                if labeling[gIndx[iAt]] == str(ilab):
                    ...
                    poscar.write(" {}\n".format("  ".join(
                        ["{0: .8f}".format(i) for i in out_array.tolist()])))
...
def _make_structures(args):
    ...
    for structure in structure_data:
        ...
        # the structure
        space_data = _map_enumStr_to_real_space(
            system, structure, args["mink"])

        space_data["aBas"] = _cartesian2direct(
            space_data["sLV"], space_data["aBas"], system["eps"])
...
def examples():
    ...
    output = (
        "RETURNS: A vasp style POSCAR labeled vasp.* where the `*` is replaced "
...
def run(args):
    ...
    if args["structures"] is not None:
        if not RepresentsInt(
                args["structures"][0]) and args["structures"][0].lower() == "all":
            ...
            raise ValueError(
                "Please enter a single structure number, two structures that "
